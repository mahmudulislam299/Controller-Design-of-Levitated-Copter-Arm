 ...
 */
// Arduino Code for arm fixed angle with kp
...
#define in1 6 //connect motor driver pin arduino 6
#define in2 7
...
String config_message = "&A~Desired~5&C&S~K_P~P~0~10~0.05&S~Motor_Speed(PWM)~O~0~260~0.01&S~Desired_Angle~A~-130~130~1&T~Arm_Angle~S1~-130~130&T~Error~S1~-150~150&T~MotorCmd~U1~0~260&H~2&";
...
void loop() {
  ...
  if (loop_counter == numSkip) {
    loop_counter = 0;
    printStatus(int(Vangle), int(error), motorCmd, headroom);
  } else loop_counter += 1;
...
void printStatus(int angle, int error, int mCmd, int hdrm) {
  char *d;
  d = buf;
  // Start Byte.
  *d = byte(0);
