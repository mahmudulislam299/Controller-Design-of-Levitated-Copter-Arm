 ...
 */
// Arduino Code for arm fixed angle with kp, kd, ki
...
int angleSensorPin = A5;
int motorEnablePin = 9;  // Do not change this!!
...
String config_message = "&A~Desired~5&C&S~K_P~P~0~20~0.05&S~K_D~D~0~10~0.05&S~K_I~I~0~20~0.05&S~SumMax~S~0~5000~1&S~Motor_Speed(PWM)~O~0~260~0.01&S~Desired_Angle~A~-130~130~1&T~Arm_Angle~S1~-130~130&T~Error~S1~-150~150&T~Delta~S2~-1000~1000&T~Sum~S2~-1000~1000&T~MotorCmd~U1~0~260&H~2&";
...
void setup() {
  ...
  setPwmFrequency(motorEnablePin, 1); // Set pwm to approx 30khz.
...
void loop() {
  ...
  if (loop_counter == numSkip) {
    loop_counter = 0;
    printStatus(int(Vangle), int(error), int(delta),int(sum), motorCmd, headroom);
  } else loop_counter += 1;
...
char buf[11];  // nine bytes to hold data for host.
// Packs up a 9-byte status package for host.
void printStatus(int angle, int error, int delta, int sum, int mCmd, int hdrm) {
  char *d;
  d = buf;
  // Start Byte.
  *d = byte(0);
  ...
  *d = byte(255);
  ...
  Serial.write(buf, 11);//for 11 byte message.
}
...
void timeSyncInit() {
  ...
  if (motorEnablePin == 5 || motorEnablePin == 6) {
    scaleT = 64;
  } else {
    scaleT = 1;
  }
